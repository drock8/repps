...
**Status.** Data store + query kernel + builder + MCP tools shipped. Builder pulls edges from `attack_surface.json` (surface-contains-endpoint, subdomain-hosts-endpoint, surface-references-tech, surface-references-js_file, surface-leaks-secret_marker) and the schema corpus (openapi_spec-documents-endpoint, endpoint-claims_auth-scheme). `bounty_build_surface_graph` and `bounty_query_surface_graph` are orchestrator-only; the build tool is idempotent via edge_hash so re-runs produce the same on-disk file. JS-extraction edges, HTTP-audit-observed edges, and brief integration (surface_graph_slice in hunter brief → C1) are pending.
