...
**Status.** Engineering-complete pending parishioner gate. The full path ships in this branch: regex-based route extractor (IP6) → symbol-to-surface index (I3) → unified diff parser (IP3) → `bounty_summarize_diff_impact` (C3 entry point). Orchestrator workflow: extract routes from authorized public repo via `bounty_extract_routes`, build the index via `bounty_build_symbol_surface_index`, then for each diff (webhook payload or `git diff` output) call `bounty_summarize_diff_impact` to get back the `impacted_surface_ids` to feed into `bounty_start_wave` for a focused regression hunt. AST-grade extraction is queued for the cases the regex misses; an actual repo-poller (cron-style watcher) is operator-driven for first slice — the orchestrator can poll periodically or accept diffs from a webhook tool the operator configures externally. Parishioner gate stays open until C3 produces ≥1 regression finding on a real target's recent PR.
