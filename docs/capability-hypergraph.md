...
**Status.** Kernel shipped (`mcp/lib/auth-differential.js`). `computeResponseSignature(observed)` classifies each response into `(status, status_class, response_class, body_shape, body_hash, body_length_bucket, sensitive_field_count, sent_with_auth)` and `diffResponseSignatures({signatures_by_profile, profile_metadata?})` emits six divergence types tagged with one of the standard severity classes. The `unauth_succeeds_where_auth_blocked` heuristic uses optional `profile_metadata.sent_with_auth` to flag genuine auth bypasses as `security`, distinct from generic doc/infra-class divergences. Runner that fans `bounty_http_scan` across multiple auth profiles, persistence to `auth-differential-results.json`, and MCP tool wrapper pending.
...
- `/bob-debug --diff-attempts <prev> <curr>` — cross-attempt evidence diff. Pending.
- `bob-export` replay-budget summary — per-pack lease usage and remaining budget. Pending.
