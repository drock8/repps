...
**Status.** Data store + query kernel shipped (`mcp/lib/surface-graph.js` + `test/surface-graph.test.js`, 10 tests passing). `appendEdges` upserts canonical edges by content hash to `surface-graph.jsonl`; `queryEdges` filters by source/target type and id and edge_type with a 1000-edge cap; `neighbors` returns incoming, outgoing, or both for a node. Builders that pull edges from `attack_surface.json`, the schema corpus, and JS-extraction artifacts plus MCP tools and brief integration are pending.
